...
const TestPage: React.FC = () => {
  ...
  const calculateMbti = (finalScores: typeof initialScores): string => {
    ...
    // 추가된 4개 지표 처리 (가정: C/D, O/U, L/M, K/B)
    mbti += finalScores.C >= finalScores.D ? 'C' : 'D';
    mbti += finalScores.O >= finalScores.U ? 'O' : 'U';
    mbti += finalScores.L >= finalScores.M ? 'L' : 'M';
    mbti += finalScores.K >= finalScores.B ? 'K' : 'B';
    return mbti;
  };
